...
class Solution:
    def minimumTime(self, n: int, relations: List[List[int]], time: List[int]) -> int:
        ...
        completion_time = time.copy()  # here, we're not adding a zero at the front
        ...
        while queue:
            ...
            for neighbor in graph[course]:
                ...
                # Adjust index by -1 since 'completion_time' is zero-indexed
                # The time to start a course is the max of the current recorded time and the completion time of its prerequisite
                completion_time[neighbor - 1] = max(completion_time[neighbor - 1], completion_time[course - 1] + time[neighbor - 1])
                if in_degree[neighbor] == 0:
                    queue.append(neighbor)
        
        # The result is the maximum completion time among all courses
